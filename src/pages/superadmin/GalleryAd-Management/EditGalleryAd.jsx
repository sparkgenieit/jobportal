...
export default function EditGalleryAd() {
  const { id } = useParams();
  ...
  useEffect(() => {
    document.title = 'Edit Gallery Ad';
    fetchAd();
  }, []);

  const fetchAd = async () => {
    try {
      const res = await http.get(`/gallery-ad/${id}`);
      setForm(res.data);
      if (res.data.blockImage)
        setPreviewBlock(`${BASE_API_URL}/uploads/gallery-ads/${res.data.blockImage}`);
      if (res.data.hoverImage)
        setPreviewHover(`${BASE_API_URL}/uploads/gallery-ads/${res.data.hoverImage}`);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await http.patch(`/gallery-ad/${id}`, formData, {
        headers: {
      ...
      setMessage('Gallery Ad updated successfully!');
      setShowMsg(true);
      ...
      setMessage(err?.response?.data?.message || 'Failed to update ad');
      setShowMsg(true);
  ...
  return (
    <div className="container-md pt-4">
      <h3 className="fw-bold text-center fs-4">Edit Gallery Advertisement</h3>
      ...
      <form onSubmit={handleSubmit} ref={formRef} encType="multipart/form-data" className="form-sample pt-3">
        ...
        <div className="col-md-9">
          <div className="form-group row">
            ...
            <div className="col-sm-9">
              ...
              />
              {previewBlock && (
                <img src={previewBlock} alt="Block" className="mt-2" width={180} height={150} />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="col-md-9">
          <div className="form-group row">
            ...
            <div className="col-sm-9">
              ...
              />
              {previewHover && (
                <img src={previewHover} alt="Hover" className="mt-2" width={180} height={150} />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="row mt-4">
          <div className="col-md-9">
            <button type="submit" className="btn btn-gradient-primary">Update</button>
